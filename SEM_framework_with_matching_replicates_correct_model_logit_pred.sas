/*Testing matching*/
LIBNAME res 'C:\Users\webst\OneDrive\Documents\PS Caliper simulations\Results_SEM_3';
...
%macro pnu_match(inDS=_predicted, replace=N, matchVar=logitpred, caliper=, matchMethod=exact,
   ...
   data &outds;
      ...
      bene_id=expID; exposure=1; DaysOn=daysOn_exposure; ps_&matchVar=expMatchVar; odds=exp(expMatchvar); ps_pred=odds/(1+odds); output;
      bene_id=match_id; exposure=0; DaysOn=daysOn_comparator; ps_&matchVar=match_matchVar; odds=exp(match_matchVar); ps_pred=odds/(1+odds); output;

      keep bene_id exposure exposureGroup daysOn ps_&matchVar posFlag ps_pred;
   run;
...
DATA trialrun;
	DO replicate=&icon;
		CALL streaminit(1250+&icon);
		DO ID=1 to 450;
			C1=RAND('BERNOULLI',0.5);
			IF C1=1 THEN C2=RAND('BERNOULLI',0.4);
			ELSE IF C1=0 THEN C2=RAND('BERNOULLI',0.1);
			C3=RAND('NORMAL',0,1);

			Xodds=log(0.15)+log(1.3)*C1+log(1.3)*C2+log(1.15)*C3;
			expodds=exp(Xodds);
			Xprob=expodds/(1+expodds);
			X=RAND('BERNOULLI',XPROB);

			YPROB=0.08+0.05*C1+0.05*C2+0.015*C3+0.05*X;
			Y=RAND('BERNOULLI',YPROB);
			OUTPUT;
		END;
	END;
RUN;
...
DATA withpredsa;
	SET withpreds;
	oddspred=pred/(1-pred);
	logitpred=log(pred);
RUN;

PROC MEANS DATA=withpredsa NOPRINT;
	VAR logitpred;
...
%pnu_match(inDS=formatformatch, replace=N, matchvar=logitpred, caliper=&calip001, matchMethod=exact,
                 outds=_matched0_001SD,order=firstlast);

%pnu_match(inDS=formatformatch, replace=N, matchvar=logitpred, caliper=&calip005, matchMethod=exact,
                 outds=_matched0_005SD,order=firstlast);

%pnu_match(inDS=formatformatch, replace=N, matchvar=logitpred, caliper=&calip01, matchMethod=exact,
                 outds=_matched0_01SD,order=firstlast);

%pnu_match(inDS=formatformatch, replace=N, matchvar=logitpred, caliper=&calip02, matchMethod=exact,
                 outds=_matched0_02SD,order=firstlast);

%pnu_match(inDS=formatformatch, replace=N, matchvar=logitpred, caliper=&calip05, matchMethod=exact,
                 outds=_matched0_05SD,order=firstlast);

%pnu_match(inDS=formatformatch, replace=N, matchvar=logitpred, caliper=&calip1, matchMethod=exact,
                 outds=_matched0_1SD,order=firstlast);

%pnu_match(inDS=formatformatch, replace=N, matchvar=logitpred, caliper=&calip, matchMethod=exact,
...
%loopit(startloop=1,endloop=1000);
