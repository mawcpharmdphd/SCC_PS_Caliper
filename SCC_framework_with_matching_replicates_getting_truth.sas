/*Testing matching*/
LIBNAME res 'C:\Users\webst\OneDrive\Documents\PS Caliper simulations\Truth';
...
%createSCCsim(name=trialrun,n=90000,repstart=&icon,repend=&icon,seed=1250+&icon, /*We'll create 1 dataset named "trialrun" with 100,000 people, with 1 replicate and a seed of 1250*/
...
PROC SQL;
	create table po&data as
		select a.bene_id, a.exposure, b.X1Y, b.X0Y from
			&data as a inner join trialrun as b
				on a.bene_id = b.ID;
QUIT;

PROC FREQ DATA=po&data NOPRINT;
	TABLES X1Y*X0Y / OUT=tab&data;
RUN;

DATA res.tab&data;
	SET tab&data;
	RETAIN doomed caused prevented immune;
	IF X0Y=1 AND X1Y=1 THEN doomed=SUM(count);
	IF X0Y=0 AND X1Y=1 THEN caused=SUM(count);
	IF X0Y=1 AND X1Y=0 THEN prevented=SUM(count);
	IF X0Y=0 AND X1Y=0 THEN immune=SUM(count);
	IF X0Y = 1 AND X1Y=1 THEN DO;
		IF caused = . THEN caused = 0;
		IF prevented = . THEN prevented = 0;
	END;
	totpop=SUM(doomed,caused,prevented,immune);
	RD=(caused - prevented)/totpop;
	RR=(caused+doomed)/totpop / ( (doomed + prevented)/totpop);
RUN;

PROC PRINT DATA=res.tab&data;
	VAR RD RR;
RUN;

%mend;

%gettruth(data=_matched0_001);

%gettruth(data=_matched0_01);

%gettruth(data=_matched0_001SD);

%gettruth(data=_matched0_005SD);

%gettruth(data=_matched0_01SD);

%gettruth(data=_matched0_02SD);

%gettruth(data=_matched0_05SD);

%gettruth(data=_matched0_1SD);

%gettruth(data=_matchedSD);
