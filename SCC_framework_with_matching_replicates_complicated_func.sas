/*Testing matching*/
LIBNAME res 'C:\Users\webst\OneDrive\Documents\PS Caliper simulations\Results_complicated_function';
...
%createSCCsim(name=trialrun,n=450,repstart=&icon,repend=&icon,seed=1250+&icon, /*We'll create 1 dataset named "trialrun" with 100,000 people, with 1 replicate and a seed of 1250*/
					...
					SCCY=%QUOTE(IF ((C1 = 1 AND PC1Y = 1) OR (C2 = 1 AND PC2Y = 1) OR ( (PC3Y * C3) > 6) OR (X=1 AND PXY=1 AND C1=1) OR (INPUTY = 1)) AND NOT (X=1 AND PXY=1 AND C2=1) ),
					INPUTC3=0.5);
